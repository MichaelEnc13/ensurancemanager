...
class Dashboard
{
    ...
    public static function get_mantenaince()
    {

        $values = array($_SESSION['user']['id']);
        return Db::queries("SELECT * FROM mantenaince WHERE uid = ?", $values);
    }

    public static function get_mantenaince_soon()
    {
        $expireSoon = 0;
        $date1 = "";
        $date2 = "";
        $values = array($_SESSION['user']['id']);
        $status = Db::queries("SELECT * FROM mantenaince WHERE uid = ?", $values)['data']->fetchAll();
